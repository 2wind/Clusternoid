...
public class CharacterManager : MonoBehaviour {
    ...
    private void FixedUpdate()
    {
        if (alive)
        {
            ...
            else if (Vector2.Distance(PlayerController.groupCenter.GetComponent<PlayerController>().centerOfGravityCharacter.GetComponent<Rigidbody2D>().position, rb.position) < maximumDistance)
            {
                var direction = Vector2.zero;
